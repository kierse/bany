...
internal class YnabAccountMapperTest {
    ...
    fun toAccount__checking() {
        ...
        assertEquals(Account("id", "name", 10L, false, Account.Type.CHECKING), YnabAccountMapper().toAccount(ynabAccount))
    }

    @Test
    fun toAccount__savings() {
        val ynabAccount = YnabAccount("id", "name", false, 10L, "savings")
        assertEquals(Account("id", "name", 10L, false, Account.Type.SAVINGS), YnabAccountMapper().toAccount(ynabAccount))
    }

    @Test
    fun toAccount__cash() {
        val ynabAccount = YnabAccount("id", "name", false, 10L, "cash")
        assertEquals(Account("id", "name", 10L, false, Account.Type.CASH), YnabAccountMapper().toAccount(ynabAccount))
    }

    @Test
    fun toAccount__credit_card() {
        val ynabAccount = YnabAccount("id", "name", false, 10L, "creditCard")
        assertEquals(Account("id", "name", 10L, false, Account.Type.CREDIT_CARD), YnabAccountMapper().toAccount(ynabAccount))
    }

    @Test
    fun toAccount__line_of_credit() {
        val ynabAccount = YnabAccount("id", "name", false, 10L, "lineOfCredit")
        assertEquals(Account("id", "name", 10L, false, Account.Type.LINE_OF_CREDIT), YnabAccountMapper().toAccount(ynabAccount))
    }

    @Test
    fun toAccount__other_asset() {
        val ynabAccount = YnabAccount("id", "name", false, 10L, "otherAsset")
        assertEquals(Account("id", "name", 10L, false, Account.Type.OTHER_ASSET), YnabAccountMapper().toAccount(ynabAccount))
    }

    @Test
    fun toAccount__other_liability() {
        val ynabAccount = YnabAccount("id", "name", false, 10L, "otherLiability")
        assertEquals(Account("id", "name", 10L, false, Account.Type.OTHER_LIABILITY), YnabAccountMapper().toAccount(ynabAccount))
    }

    @Test
    fun toAccount__pay_pal() {
        val ynabAccount = YnabAccount("id", "name", false, 10L, "payPal")
        assertEquals(Account("id", "name", 10L, false, Account.Type.PAY_PAL), YnabAccountMapper().toAccount(ynabAccount))
    }

    @Test
    fun toAccount__merchant_account() {
        val ynabAccount = YnabAccount("id", "name", false, 10L, "merchantAccount")
        assertEquals(Account("id", "name", 10L, false, Account.Type.MERCHANT_ACCOUNT), YnabAccountMapper().toAccount(ynabAccount))
    }

    @Test
    fun toAccount__investment_account() {
        val ynabAccount = YnabAccount("id", "name", false, 10L, "investmentAccount")
        assertEquals(Account("id", "name", 10L, false, Account.Type.INVESTMENT_ACCOUNT), YnabAccountMapper().toAccount(ynabAccount))
    }

    @Test
    fun toAccount__mortgage() {
        val ynabAccount = YnabAccount("id", "name", false, 10L, "mortgage")
        assertEquals(Account("id", "name", 10L, false, Account.Type.MORTGAGE), YnabAccountMapper().toAccount(ynabAccount))
    }
