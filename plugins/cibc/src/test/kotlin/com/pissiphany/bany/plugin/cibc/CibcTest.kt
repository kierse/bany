...
internal class CibcTest {
    private val BASE_URL = "https://api.ebanking.cibc.com"
    private val APP_CONFIG_URL = "$BASE_URL/ebm-resources/public/client/mobile/conf/appconfig-cibc.json"
...
//        seedCookieJar(client)
    ...
    private fun fetchAppConfig(client: OkHttpClient) {
        ...
        println()
        println("fetch app config request")
        println(request.toString())
        printHeaders(request.headers())

        println()
        println("fetch app config response")

        client.newCall(request).execute().use { response ->
            if (response.code() != 200) throw Exception("error!")
        }
    }
